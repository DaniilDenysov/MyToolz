...
namespace MyToolz.IO
{
    ...
    public abstract class SaveLoadBase<T> : MonoInstaller, ISaver<T> where T : class, new()
    {
        ...
        [FoldoutGroup("Status"), ShowInInspector, ReadOnly, LabelText("Folder (Resolved)")]
        private string _resolvedFolder => _resolvedFolderCache;

        [FoldoutGroup("Status"), ShowInInspector, ReadOnly, LabelText("Full Path")]
        private string _fullPathPreview => _fullPath;

        [FoldoutGroup("Status"), ShowInInspector, ReadOnly, LabelText("File Exists")]
...
#else
            DebugUtility.LogWarning("Copy path is only available in the Editor.");
#endif
        }
