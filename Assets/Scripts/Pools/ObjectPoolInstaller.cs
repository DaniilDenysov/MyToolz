...
namespace MyToolz.Pools
{
        ...
        where T : MonoBehaviour where P : Pool<T>
    {
        ...
        private void OnPoolRequestReceived(PoolRequest<T> request)
        {
            ...
            {
                DebugUtility.LogWarning($"PoolRequest failed: {e}");
            }
        }
        ...
        private void OnReleaseRequestReceived(ReleaseRequest<T> request)
        {
            ...
            {
                DebugUtility.LogWarning($"ReleaseRequest failed: {e}");
            }
        }
        ...
        public virtual T Get(T prefab)
        {
            if (prefab == null)
            {
                DebugUtility.LogError("Provided prefab is null.");
                return null;
            }
            ...
            {
                DebugUtility.LogWarning($"No pool found for prefab: {prefab.name}");
            }
        ...
        public virtual void Release(T obj)
        {
            if (buffer.TryGetValue(obj, out int prefabId))
            {
                if (mappings.TryGetValue(prefabId, out var pool))
                {
                    DebugUtility.Log($"ReleaseRequest fullfilled!");
                    buffer.Remove(obj);
                    pool.Despawn(obj);
                }
                else
                {
                    DebugUtility.LogWarning("No pool found for prefabId: " + prefabId);
                }
            }
            else
            {
                DebugUtility.LogWarning("Failed to get prefabId for object: " + obj.name);
            }
        }
    }
}