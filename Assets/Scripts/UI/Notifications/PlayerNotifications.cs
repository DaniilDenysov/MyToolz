...
{
    public class PlayerNotifications : MonoBehaviourPlus
    {
        ...
        private void InitializePrefabMap()
        {
            ...
            foreach (var prefab in notificationPrefabs)
            {
                ...
                if (keyType == null)
                {
                    LogWarning("Notification prefab has null message type (strategy missing?)");
                    continue;
                }

                if (!prefabMap.TryAdd(keyType, prefab))
                {
                    LogWarning($"Duplicate notification prefab mapping for type: {keyType}");
                }
            }
        }
        ...
        private void OnNotificationClearRequest(NotificationClearRequest e)
        {
            ...
            {
                LogWarning("NotificationClear.Key is null/empty");
                return;
            }
        ...
        private void OnNotificationRequested(NotificationRequest e)
        {
            ...
            {
                LogError("NotificationRequestEvent.MessageType is null");
                return;
            }

            if (!prefabMap.TryGetValue(e.MessageType, out var prefab))
            {
                LogWarning($"No notification prefab mapped for type: {e.MessageType}");
                return;
            }
